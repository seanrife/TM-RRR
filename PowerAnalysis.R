...
##### SET UP SIMULATION USING CONSERVATIVE PARAMETERS #####
...
nSims <- 13 #number of participating labs
...
for (n in 1:nLoops){
  ...
  for(i in 1:nSims){ #for each simulated experiment
    ...
    SSn1[i]<-round(SampleSize*.25) #save sample size group 1
    SSn2[i]<-round(SampleSize*.75) #save sample size group 2
    es.d[i]<-smd(Mean.1= mean(x), Mean.2=mean(y), s.1=sd(x), s.2=sd(y), n.1=round(SampleSize*.25), n.2=round(SampleSize*.75), Unbiased=FALSE) #Using Cohen's d due to large(ish) sample
  }
